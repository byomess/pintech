...
const ToolCard: React.FC<{
  ...
  toggleFavorite
    ...
    return (
      <div key={name} className="break-inside-avoid p-4 bg-gray-700 rounded-lg shadow-md border border-solid border-gray-600 hover:border-white flex flex-col mb-4 overflow-hidden">
        <div className="flex items-start space-x-4">
          {logoPath && (
            <div className="flex-shrink-0">
              <a target="_blank" href={url}>
                <Image src={logoPath} alt={`${name} logo`} className="w-16 h-16" width={256} height={256} />
              </a>
            </div>
          )}
          <div className="flex flex-col flex-1 min-w-0">
            ...
            <div ref={draggableScrollRef} className="cursor-grab active:cursor-grabbing flex flex-1 mt-2 overflow-auto hide-scrollbar gap-1" style={{ whiteSpace: 'nowrap' }}>
              {tags.map((tag, index) => (
  ...
  return (
    <div className="w-full lg:w-9/12 p-4">
      <div className="container mx-auto">
        <h1 className="text-3xl font-bold text-center mb-4 hidden sm:block"><span className="text-rose-600">pin</span>tech</h1>
        <div className="mb-4">
          <input
